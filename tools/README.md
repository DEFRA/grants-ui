...
Choose **tools ⇢** for **application state**, **journey ⇢**, **audit logs** (`npm run audit:logs`, audit entries from grants-ui container logs), **audit queue** (`npm run audit:queue`, the 10 most recent local audit messages), and **clear audit** (`npm run audit:clear`, purge the local audit queue and restart grants-ui). When the GAS addon is running, **manage GAS ⇢** lists submitted applications by grant code and client reference with their current phase, stage and status. Selecting an application opens these actions:

- **change status** offers statuses defined by the application's grant version and updates all three GAS state fields together. This is a direct local database edit; it does not run GAS processes.
- **generate offer** reads the application's latest database state and finds a Caseworking mapping whose valid transition runs `GENERATE_OFFER`. It resolves the latest active config within the application's major version, using the grant definition cached by GAS. It sends an SQS event with generated IDs through the running GAS container's AWS SDK and queue configuration. No host AWS CLI or test endpoint is needed. Success means the event was queued; agreement creation happens asynchronously. Inspect GAS logs and its inbox/outbox for processing failures.
- **prepare claim** is a local woodland fixture shortcut for an application at `STATUS_AGREEMENT_READY_FOR_APPLICANT`: it sends the Caseworking offer and completion events, simulates the Agreement Service acceptance event, directly sets `STATUS_PREPARING_CLAIM`, creates the PA3 entitlement through GAS's authenticated grant-admin API, then sets `STATUS_AWAITING_CLAIM`. It uses the application’s selected woodland hectares and temporarily mints an admin access token inside GAS; the raw token is never displayed or retained.

Offer generation reports an error if an agreement already exists, the current position has no offer transition, the transition is ambiguous, or the resolved definition is not cached. It does not reset statuses or replace agreements. For woodland, start at `PHASE_PRE_AWARD:STAGE_REVIEWING_APPLICATION:STATUS_APPLICATION_RECEIVED`; the submitted answers must include the data used by the agreement definition (including `payments.agreement` with `PA3`). Both actions honour `--dry-run`. The GAS container defaults to `gas`; override it with `GRANTS_UI_GAS_CONTAINER` if needed.

Audit logs and audit queue open the output viewer automatically on completion; close the viewer to return to Tools. Submenus stay open after each run; press **l** to view output or **Escape** to go back. The runtime footer refreshes when returning to Tools.

Choose **tools ⇢ → journey ⇢** to automatically fill a grant journey in the background or watch it in Chrome. The setup lets you choose a journey, sign-in CRN, saved-state handling and applicable journey options; in Chrome you can also choose a page to stop on. Containers must be running.
