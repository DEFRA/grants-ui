...
const landGrantsApiClient = createLandGrantsApiClient(LAND_GRANTS_API_URL)
...
export async function calculateGrantPayment(landParcels) {
  const payment = await landGrantsApiClient.calculate(landParcels)
...
export async function fetchAvailableActionsForParcel({ parcelId = '', sheetId = '' }) {
  ...
  const parcels = await landGrantsApiClient.parcelsWithActionsAndSize(parcelIds)
 ...
 * @param {{ sbi:string, sheetId: string, parcelId: string, actionsObj: {[code: string]: {description: string, value: string, unit: string}} | {}}} payload
 * @returns {Promise<object>} - Validation result
 * @throws {Error}
 */
export async function triggerApiActionsValidation({ sbi, sheetId, parcelId, actionsObj = {} }) {
  const apiPayload = landActionsToApiPayload({ sheetId, parcelId, actionsObj, sbi })

  return await landGrantsApiClient.validateActions(apiPayload)
}
...
export async function validateApplication(data) {
  ...
  return landGrantsApiClient.validateApplication(payload)
}

/**
 * @import { LandActions } from './land-grants-api-client.d.js'
 */
