...
describe('landGrantsActionsPlugin', () => {
  ...
  describe('POST /api/land-grants/actions/{parcelId}', () => {
    ...
    const validatePlannedActionUnit = (unit) =>
      routeFor(AVAILABILITY_PATH).options.validate.payload.validate({
        plannedActions: [{ actionCode: 'UPL1', quantity: 5, unit }]
      }).error
    ...
    it('still validates the plannedActions payload only', () => {
      ...
      expect(
        payload.validate({ plannedActions: [{ actionCode: 'CSAM3', quantity: 1.5, unit: 'ha' }] }).error
      ).toBeUndefined()
