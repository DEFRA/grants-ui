...
Both metrics tracking and the display panel only run when the page opts in. See the `basemap-metrics` attribute below.
...
### 1. Get an OS Maps API key

The basemap always comes from Ordnance Survey, **including in mock mode**: mock mode only replaces the parcel geometry, never the map underneath it. Without a key the basemap 401s and the component shows its error overlay, which looks much like the map being broken. See [OS Maps API key](#os-maps-api-key) below for how to mint one, then put it in `.env`:
...
### 3. Sign in as a user who actually has parcels
...
| Basemap draws, parcels missing, real mode           | `MAP_MOCK_DATA_ENABLED=false` but running `docker:up` rather than `docker:landgrants:up`.                                 |
...
| --------------------------- | -------------------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `osMapsApiKey`              | `OS_MAPS_API_KEY`                | _(none)_                               | OS Data Hub key. Sensitive, server-side only. See below.                                        |
