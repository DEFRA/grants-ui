...
#### 1. Define Confirmation Content in Form YAML

Please see src/server/common/forms/definitions/flying-pigs.yaml / find-funding-for-land-or-forms.yaml / example-grant-with-auth.yml for examples
...
If a form doesn't have `confirmationContent` defined, the system will:

- Return a 501 Not Implemented status
- Fall back to the standard forms engine confirmation handling
- Log that the form is not using config-driven confirmation
