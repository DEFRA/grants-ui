...
@runme
    Scenario: Complete a grant application with multiple users with different permissions
        Given there is no application state stored for CRN "1062311182" and SBI "106238911" and grant "example-grant-with-auth"
        ...
        # cannot-submit (CRN 1062311182 has AMEND but not SUBMIT permissions)
        Then the user should be at URL "cannot-submit"
        And should see heading "You cannot submit this application"

        # unlock and log in as CRN 1062311181 (SUBMIT permissions) in a new browser session
