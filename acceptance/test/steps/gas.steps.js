...
import { getExpectationIdStore } from '../utils/expectation-id-store.js'
...
if (process.env.MOCKSERVER_HOST) {
  Before(async function () {
    ...
    getExpectationIdStore(this).push(expectationId)
  })

  After(async function () {
    for (const expectationId of getExpectationIdStore(this).all) {
...
Given('the application status in GAS is now {string}', async function (gasStatus) {
  const referenceNumbers = getReferenceNumberStore(this)
