...
| `STATIC_CACHE_TIMEOUT`                       | Static asset cache timeout in milliseconds. Only production content-hashes asset filenames, so only production caches them; elsewhere the default is `0` (`Cache-Control: no-cache`) so a rebuilt bundle is never served stale | `604800000` (1 week) in production, else `0` |
