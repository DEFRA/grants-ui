...
Once the current time reaches a grant's `metadata.applicationWindow.closesAt` in the form definition, new/draft applicants are redirected to the "Application window closed" page (existing applications with a submitted status are unaffected). Testing that for real means editing the date in the grant's config repo and republishing it — awkward for a quick check.

```sh
gt journey example-grant-with-auth --mock-window-closed --headed    # or pick "Window closed" in the TUI
```

This sets a `dev_mock_window_closed=1` cookie, honoured by `resolveApplicationWindowClosedPath` and `serviceRootRedirect` (`src/server/common/request-pipeline/redirects/`) as if the grant's application window were closed, regardless of its configured `closesAt`. It's read through `isWindowClosedMockEnabled()` (`src/server/common/helpers/mock-overrides.js`), which returns `false` whenever `devTools.enabled` is off — so the cookie is inert in a deployed environment.
