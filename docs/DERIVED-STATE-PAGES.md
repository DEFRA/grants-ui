...
Use `withDerivedState` from `src/server/common/helpers/state/with-derived-state.js` for pages that calculate local answers
...
- `getCalculatedAnswers(request, state)` returns an object or a promise for that object. It must not mutate state, persist
  answers, redirect, submit a grant application, or call a calculation API. Check answers invokes this method to compare
  local calculated outputs with saved answers.
...
- `await isStateStale(request, context)` performs a read-only check. Missing owned answers are stale. Otherwise, it
  compares the calculated outputs structurally with the saved values. Use this for deterministic,
  inexpensive local calculations such as total estimated cost.
